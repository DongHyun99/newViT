...
class PatchEmbedding(nn.Module):
    def __init__(self, in_channels: int = 3, patch_size: int = 16, emb_size: int = 768, img_size: int = 224, dimension: bool = False):
        ...
        if dimension == True:
            x_patches = torch.arange(1, img_size//patch_size+1)
            y_patches = torch.arange(1, img_size//patch_size+1)
            ...
            self.positions = sinusoidal_PE_2d(num_patches_2d, emb_size)
...
class visionTransformer(nn.Sequential):
    def __init__(self,     
                ...
                **kwargs):
        super().__init__(
            PatchEmbedding(in_channels, patch_size, emb_size, img_size, dimension),
